 ...
 * suppresses the bean.
 *
 * <p>The OUT_OF_SERVICE test uses a private breaker rather than the
 * autowired bean so the global circuit-breaker state isn't disturbed —
 * other tests sharing this Spring context (notably
 * {@link EndToEndWiringIT}) would be brittle if we transitioned the
 * shared breaker through OPEN/CLOSED inside another test method.
 */
...
class HealthIndicatorIT extends EndToEndTestBase {
    ...
    void healthIndicator_isUp_whenRedisReachable() {
        // Touch a cache so the cache details map has at least one entry.
        productService.findById(1L);

        Health health = healthIndicator.health();
        assertThat(health.getStatus()).isEqualTo(Status.UP);
        assertThat(health.getDetails())
                .containsKeys("breakerState", "redisPing", "redisPingMs", "caches");
        assertThat(health.getDetails().get("breakerState")).isEqualTo("CLOSED");
    ...
    void healthIndicator_isOutOfService_whenBreakerOpen() {
        // Construct a private breaker + indicator so we don't disturb the
        // shared `redisCacheCircuitBreaker` bean used by NearCache instances
        // in other tests sharing this context.
        CircuitBreaker privateBreaker = CircuitBreakerRegistry.ofDefaults()
                .circuitBreaker("health-it-" + System.nanoTime());
        ...
        assertThat(health.getDetails().get("breakerState")).isEqualTo("OPEN");
    }
